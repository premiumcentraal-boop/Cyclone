...
V3 remains the running product (3.5.x). V4 is the next infrastructure layer on top of V3, not a rewrite.
...
- goal-ranked interactive candidates (not “first 12 of an arbitrary list”)
- last transition
...
Everything else is advanced and hidden unless doctor says it is available. Permission-aware tool lists: omit tools the current phone cannot run.

Official agent instructions live at `tools/codex-phone-mcp/SKILL.md`. Reprint that file from the package so it cannot drift from code.
...
## What to copy from other apps (patterns only)

| Source | Take | Leave |
|---|---|---|
| phone-harness | `SKILL.md`, doctor stop-the-agent, verify-after-tap | ADB `input tap` as production hands |
| android-remote-control-mcp | page-state cursor, WebView collapse, placeholder round-trip, driving banner | Second on-phone MCP server / LAN bind |
| Mobilerun (MIT) | app cards, manager→executor, traces | CodeAct Python-on-device, AGPL Portal source |
| Gemini UI automation | overlay verbs and live view | OEM lock-in, single-model runtime |

## Lab method

Do not start V4 by inventing an emulator farm.

1. Capture golden pages on the Pixel 8 via Teach → Page Awareness Sandbox into `vault-lab/golden-pages/`.
2. Contract-test page card, act envelope, skill compile against those fixtures.
3. Optional AVD smoke (skip-clean if the image is missing).
...
AndroidWorld is a later gate, not the brand.

## Ownership for V4 work

| Lane | Paths |
|---|---|
| Overlay UX | `apps/mobile/.../ui/**`, Accessibility overlay controller |
| Page card | MCP `compact.py`, `observe.semantic`, PageSignatureEngine |
| Act envelope | `PhoneToolExecutor`, gateway action, MCP `phone_act` |
| Skills | `automation/**`, `brain/memory`, routine capsules |
| Policy | `policy/**` GATE |
| Fleet | Device Gateway `device_id`, Companion wall |

One agent per lane unless contracts are frozen first.
