...
class ArtemisSubgoal(BaseModel):
    """Upsert ArtemisSubgoal API model."""
...
class UpsertTaskRunPlanRequest(BaseApiModel):
    """Upsert task run plan request model."""

    started_at: IsoDatetime = Field(..., description="When the plan started")
    subgoals: list[ArtemisSubgoal] = Field(..., description="Subgoals of the plan")
