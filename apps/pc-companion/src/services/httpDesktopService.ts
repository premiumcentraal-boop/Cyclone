...
export class HttpDesktopService implements DesktopService {
  ...
  setDeviceNickname(deviceId: string, nickname: string): Promise<{ deviceId: string; nickname: string | null }> {
    return this.request(`/v1/fleet/devices/${encodeURIComponent(deviceId)}/nickname`, {
  ...
  listDeviceTasks(missionId?: string): Promise<DeviceTask[]> {
    ...
    return this.request<{ tasks: DeviceTask[] }>(`/v1/fleet/tasks${suffix}`).then((r) => r.tasks);
  }
  ...
  runRootCommand(request: RootCommandRequest): Promise<RootCommandResult> {
    return this.request("/v1/fleet/command", {
      method: "POST",
      body: JSON.stringify({
        command: request.command, model: request.model, providers: request.providers,
        apiKey: request.apiKey, deviceIds: request.deviceIds,
      }),
    });
  }

  listScenes(): Promise<Scene[]> {
    return this.request<{ scenes: Scene[] }>("/v1/fleet/scenes").then((r) => r.scenes);
  }

  saveScene(sceneId: string, name: string, steps: SceneStep[]): Promise<Scene> {
    return this.request(`/v1/fleet/scenes/${encodeURIComponent(sceneId)}`, {
      method: "POST", body: JSON.stringify({ name, steps }),
    });
  }

  deleteScene(sceneId: string): Promise<void> {
    return this.request(`/v1/fleet/scenes/${encodeURIComponent(sceneId)}/delete`, { method: "POST" }).then(() => undefined);
  }

  runScene(sceneId: string, credentials: { model: string; providers: string[]; apiKey: string }): Promise<RootCommandResult> {
    return this.request(`/v1/fleet/scenes/${encodeURIComponent(sceneId)}/run`, {
