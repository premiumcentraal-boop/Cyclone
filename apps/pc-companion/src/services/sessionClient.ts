...
export type CycloneOneSessionState =
  | "FOREGROUND"
  | "RUNNING"
  | "PAUSED"
...
export interface CycloneOneSession {
  sessionId: string;
  displayId: number;
  targetPackage?: string | null;
  backend?: string;
  inputOwner?: string;
  state: CycloneOneSessionState;
  executable?: boolean;
  executionGeneration?: number | null;
  frameHealthy?: boolean | null;
}

export interface CycloneOneSessionList {
  protocol: string;
  deviceId: string;
  sessions: CycloneOneSession[];
}

export interface CycloneOneSessionResult {
  protocol: string;
  deviceId: string;
  session: CycloneOneSession;
}
...
export class CycloneOneSessionClient {
  ...
  async snapshot(deviceId: string, sessionId: string): Promise<{ url: string; displayId: number }> {
    ...
    const exact = String(response.headers.get("X-Cyclone-Foreground-Substitution") || "").toLowerCase();
    if (exact !== "false") throw new Error("Cyclone refused an unproven background preview");
    const displayRaw = response.headers.get("X-Cyclone-Display-Id") || "";
