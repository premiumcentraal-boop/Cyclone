...
class Repository:
    ...
    async def create_conversation(self, *, title: str, kind: str, project_key: str | None, agent_slugs: list[str]) -> ConversationDetail:
        async with self.connection() as connection:
    ...
    async def get_conversation(self, conversation_id: UUID, message_limit: int = 200) -> ConversationDetail:
        async with self.connection() as connection:
            ...
            messages_result = await connection.execute("""
                SELECT m.id, m.conversation_id, m.task_id, m.author_type, m.author_agent_id,
            ...
            messages = [self._message(row) for row in await messages_result.fetchall()]
    ...
    async def add_message(self, *, conversation_id: UUID, author_type: str, body: str, kind: str = "message", author_agent_id: UUID | None = None, task_id: UUID | None = None, metadata: dict[str, Any] | None = None, source: str = "cyclone") -> Message:
        async with self.connection() as connection:
            result = await connection.execute("""
                INSERT INTO messages
                  (conversation_id, task_id, author_type, author_agent_id, kind, body, metadata, source)
                VALUES (%s, %s, %s, %s, %s, %s, %s::jsonb, %s)
                RETURNING id, conversation_id, task_id, author_type, author_agent_id,
                          kind, body, metadata, source, created_at
            """, (conversation_id, task_id, author_type, author_agent_id, kind, body, psycopg.types.json.Jsonb(metadata or {}), source))
            row = await result.fetchone()
