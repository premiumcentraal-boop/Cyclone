...
class RuleEngine(private val anchorDate: LocalDate = LocalDate.now()) {
    fun evaluate(rules:List<ShiftRule>, openShifts:List<OpenShift>):EvaluationSummary {
        ...
        rules.forEach { rule ->
            ...
            openShifts.filter { allows(rule,it.date) }.groupBy { it.date }.toSortedMap().forEach { (date, shifts) ->
                val selected=when(rule.type){ RuleType.EXACT->shifts.filter{it.code==rule.codes.single()}; RuleType.COMBINATION->shifts.filter{it.code in rule.codes}; RuleType.SEQUENCE->matchSequence(rule,shifts) }
    ...
    private fun matchSequence(rule:ShiftRule, shifts:List<OpenShift>):List<OpenShift> { val selected=shifts.filter { it.code in rule.codes }; if(rule.codes.any{c->selected.count{it.code==c}!=1}) return emptyList(); val ordered=if(selected.all{it.startTime!=null}) selected.sortedBy{it.startTime}else selected.sortedBy{it.code.order}; return if(ordered.map{it.code}==rule.codes) ordered else emptyList() }
