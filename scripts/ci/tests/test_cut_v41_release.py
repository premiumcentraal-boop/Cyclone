...
SCRIPT = Path(__file__).parents[1] / "cut_v41_release.py"
SPEC = importlib.util.spec_from_file_location("cut_v41_release", SCRIPT)
...
class CutV41ReleaseTest(unittest.TestCase):
    ...
    def test_required_identity_is_v41_and_one_1(self):
        self.assertEqual("v4.1.0", cut.TAG)
        self.assertEqual("4.1.0", cut.REQUIRED_MOBILE)
        self.assertEqual(80, cut.REQUIRED_ANDROID_VERSION_CODE)
        self.assertEqual("1.0.0", cut.REQUIRED_PC_COMPANION)
        self.assertEqual("Cyclone Mobile 4.1.0", cut.TITLE)
        self.assertEqual("docs/RELEASE_4.1.md", cut.NOTES_REL)
        self.assertEqual("release/cyclone-mobile-v4.1.0", cut.RELEASE_BRANCH)
        self.assertEqual("grok/mobile-4.1-s5-release", cut.STAGE5_BRANCH)
        self.assertEqual("grok/mobile-4.1-s4-fastpath-bg", cut.STAGE4_BRANCH)
        self.assertEqual("Cyclone-Android-4.1.0", cut.TYPICAL_ANDROID_ARTIFACT)
        self.assertEqual("Cyclone-4.1.0.apk", cut.TYPICAL_SIGNED_APK)
    ...
    def test_alpha_identity_is_rejected(self):
        ...
        metadata["mobile"] = "4.1.0-alpha.4"
        metadata["android_version_code"] = 79
        metadata["pc_companion"] = "4.1.0-alpha.4"
        errors = cut.version_errors(metadata)
        self.assertIn("mobile='4.1.0-alpha.4' expected '4.1.0'", errors)
        self.assertIn("android_version_code=79 expected 80", errors)
        self.assertIn("pc_companion='4.1.0-alpha.4' expected '1.0.0'", errors)
    ...
    def test_operator_sequence_documents_stack_ci_and_copy_artifact_name(self):
        ...
        self.assertIn("#65", steps)
        self.assertIn("#67", steps)
        self.assertIn("#69", steps)
        self.assertIn("#71", steps)
        self.assertIn("grok/mobile-4.1-s5-release", steps)
        self.assertIn("grok/mobile-4.1-s4-fastpath-bg", steps)
        self.assertIn("release/cyclone-mobile-v4.1.0", steps)
        self.assertIn("mobile-release.yml", steps)
        self.assertIn("do not invent the artifact name", steps)
        self.assertIn("Cyclone-Android-4.1.0", steps)
        self.assertIn("Do NOT dispatch pc-companion-release.yml", steps)
        self.assertNotIn("Dispatch pc-companion-release.yml", steps)
        self.assertIn("A5", steps)
        self.assertIn('gh release create v4.1.0 --title "Cyclone Mobile 4.1.0"', steps)
        self.assertIn("docs/RELEASE_4.1.md", steps)

    def test_release_create_command_never_overwrites(self):
        command = cut.build_release_create_command(
            target_sha="a" * 40,
            apk=Path("Cyclone-4.1.0.apk"),
        )
        self.assertEqual(
            [
                "gh",
        ...
        self.assertIn('--title "Cyclone Mobile 4.1.0"', rendered)
    ...
    def test_execute_blockers_cover_preflight_gates(self):
        ...
        notes = Path(temporary.name) / "RELEASE_4.1.md"
        notes.write_text("# v4.1\n", encoding="utf-8")
        metadata = self.ready_metadata()
        sha = "b" * 40
        ...
        blocked = cut.execute_blockers(
            ...
            notes_path=Path("/missing/RELEASE_4.1.md"),
        ...
        self.assertIn("local tag v4.1.0 already exists", joined)
        self.assertIn("remote tag v4.1.0 already exists", joined)
        self.assertIn("GitHub release v4.1.0 already exists", joined)

    def test_dry_run_exits_zero_when_identity_matches_even_if_unpublished(self):
        metadata = self.ready_metadata()
        stdout = io.StringIO()
        with patch.object(cut, "NOTES", Path("/missing/RELEASE_4.1.md")):
            with patch("sys.stdout", stdout):
        ...
        self.assertNotIn("gh workflow run pc-companion-release.yml", text)
        self.assertIn("Do NOT dispatch pc-companion-release.yml", text)
        self.assertNotIn("ERROR:", text)

    def test_dry_run_refuses_when_identity_is_not_v41(self):
        metadata = self.ready_metadata()
        metadata["mobile"] = "4.1.0-alpha.4"
        metadata["android_version_code"] = 79
        stdout = io.StringIO()
        ...
        self.assertIn("mobile='4.1.0-alpha.4' expected '4.1.0'", stdout.getvalue())
        self.assertIn("android_version_code=79 expected 80", stdout.getvalue())
