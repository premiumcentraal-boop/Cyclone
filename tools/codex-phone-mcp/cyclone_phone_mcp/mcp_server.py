...
TOOLS = [
    ...
    _tool("phone_ui_search", "Search the gateway's fuller UI index when a needed target is missing from compact context.", {"type": "object", "properties": {"query": {"type": "string"}}, "required": ["query"], "additionalProperties": False}, read_only=True),
    _tool("phone_inspect_element", "Inspect one UI element candidate and its semantic/accessibility evidence.", {"type": "object", "properties": {"element_id": {"type": "string"}}, "required": ["element_id"], "additionalProperties": False}, read_only=True),
    ...
    _tool("phone_act", "Execute one typed Cyclone phone action, then let the caller observe and verify. phone.type requires user_authorized=true.", {"type": "object", "properties": {"tool": {"type": "string", "enum": ["phone.click", "phone.long_press", "phone.swipe", "phone.scroll", "phone.type", "phone.back", "phone.home", "phone.open_app", "phone.wait_for"]}, "params": {"type": "object"}, "goal": {"type": "string"}, "user_authorized": {"type": "boolean", "default": False}}, "required": ["tool", "params", "goal"], "additionalProperties": False}, read_only=False, destructive=True),
    _tool("phone_debug_bundle", "Capture the 2.9.3 diagnostic bundle when perception, context, execution or verification disagree.", {"type": "object", "properties": {"goal": {"type": "string"}, "expected": {"type": "string"}}, "additionalProperties": False}, read_only=True),
