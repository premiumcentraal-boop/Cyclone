...
INSTRUCTIONS = (
    ...
    "Three planes: Foreground (session_id=default-foreground, display 0), Session Kernel VD (named session_id + display_id>0), Layer 2 workspace (workspaceId+workspaceGeneration on default-foreground/display 0). "
    "Responses include plane metadata. Read session_id from phone_status (sessions inventory when present). "
    "Named VD sessions must never be rewritten onto display 0; Layer 2 is not a VD tile. "
