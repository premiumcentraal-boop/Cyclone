...
CIP is intentionally narrower than the generic gateway. It composes the existing
LivePhone/PhoneTools authority and never exposes shell, ADB, host process, file,
...
def _bounded(value: Any, name: str, limit: int = 240, *, required: bool = False) -> str:
    ...
    return value.strip() if name != "text" else value
...
class CipEngine:
    """Transport-neutral SEE/ACT adapter over the existing foreground broker."""
    ...
    def devices(self) -> dict[str, Any]:
        ...
        rows = self._devices()
        return {"ok": True, "protocol": PROTOCOL, "devices": rows, "auto_selected": rows[0]["id"] if len(rows) == 1 else None}
    ...
    def _wrap_observation(self, raw: dict[str, Any], *, goal: str = "") -> dict[str, Any]:
        ...
        return {
            ...
            "sequence_at": int(time.time() * 1000),
    ...
    def see(self, *, device: Any = None, goal: Any = "", detail: Any = "compact") -> dict[str, Any]:
        ...
        if goal_text:
            try:
                ranked = self.live.tools.phone_ui_search({
                    "device_id": device_id,
                    "session_id": SESSION,
                    "display_id": DISPLAY,
                    "query": goal_text[:240],
    ...
    def inspect(self, *, observation_id: Any, element_id: Any, device: Any = None) -> dict[str, Any]:
        ...
        result = self.live.tools.phone_inspect_element({
            "device_id": device_id, "session_id": SESSION, "display_id": DISPLAY, "element_id": element,
        })
        return {"ok": True, "protocol": PROTOCOL, "observation_id": obs, "element": result}

    def _pixel(self, observation: dict[str, Any], point: Any, name: str) -> tuple[float, float]:
        x, y = _point(point, name)
    ...
    def _compile_action(self, observation: dict[str, Any], action: Any) -> tuple[str, dict[str, Any]]:
        if not isinstance(action, dict) or set(action) - {"kind", "target", "text", "direction", "amount", "from", "to", "duration_ms", "package"}:
        ...
        if kind in {"tap", "long_press", "type", "clear"}:
            ...
            if "element_id" in target:
                ...
                # The frozen gateway exposes coordinate selectors for click/long-press. A later
                # exact phone.tap capability may refine canvas behavior without changing CIP.
        ...
        if kind == "type":
            ...
            params["text"] = ""
        if kind == "scroll":
            direction = str(action.get("direction") or "forward").lower()
            direction = {"down": "forward", "up": "backward"}.get(direction, direction)
        ...
        if kind == "open_app":
            params["package"] = _bounded(action.get("package"), "package", required=True)
        mapping = {
            "tap": "phone.click", "long_press": "phone.long_press", "type": "phone.type",
            "clear": "phone.type", "scroll": "phone.scroll", "swipe": "phone.swipe",
            "back": "phone.back", "home": "phone.home", "open_app": "phone.open_app",
        }
    ...
    def _status(result: dict[str, Any], after: dict[str, Any] | None) -> str:
        ...
        execution = action_status.get("execution")
        error = str(result.get("errorClass") or "")
        definitive = {"POLICY_DENIED", "STALE_OBSERVATION", "CAPABILITY_UNAVAILABLE", "INVALID_REQUEST", "AUTH_REJECTED"}
        if execution == "failed" or error in definitive:
            return "FAILED"
        return "UNCERTAIN"
    ...
    def act(self, *, request_id: Any, observation_id: Any, action: Any, device: Any = None,
        ...
        # Consume authority before crossing the mutation boundary. No automatic mutation retry.
        ...
        try:
            outcome = self.live.tools._run_verified_mutation(
                device_id, tool, params, goal_text,
                scope={"sessionId": SESSION, "displayId": DISPLAY},
            )
        except Exception as exc:
            result = {
                "ok": False, "protocol": PROTOCOL, "request_id": req, "status": "UNCERTAIN",
    ...
    def session(self, operation: Any = "status") -> dict[str, Any]:
        ...
        if op not in {"status", "enable", "pause", "stop"}:
            raise CipError("INVALID_REQUEST", "session operation must be status/enable/pause/stop")
        if op == "status":
            control = self._control()
            return {"ok": True, "protocol": PROTOCOL, "operation": op, "generation": control.get("generation"), "enabled": control.get("enabled") is True, "stopped": control.get("stopped") is True}
        # Broker control is owned by Cyclone One. MCP may expose intent only when host wiring
        # explicitly authorizes it; direct engine users update the same file through One/IPC.
